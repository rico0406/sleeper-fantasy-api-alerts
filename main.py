...
def build_alerts(alert_type: str, league: League, user_id: str, logger) -> list:
    """
    ...
    logger.info(f"Building alerts for type '{alert_type}' (Last week: {last_week})")
...
def main():
    ...
    logger = get_logger(alert_type)

    try:
        logger.info("Starting alert handler...")

        league = League(league_id)
        alerts = build_alerts(alert_type, league, user_id, logger)

        if alerts:
            send_alerts(alerts)
            logger.info(f"{len(alerts)} alerts sent successfully.")
        else:
            logger.info(f"No alerts generated for type '{alert_type}'.")

    except Exception as e:
        logger.error(f"Error while running alert handler: {e}", exc_info=True)
